...
- [String Metrics](/api/string-metrics) -- distance and similarity functions used by BkTree and VpTree
- [Batch Operations](/api/batch-operations) -- `cdist`, `match_best`, and `match_batch` for brute-force comparison
- [Pipeline](/api/pipeline) -- use indexes implicitly via `block_lsh` and `block_qgram` blocking strategies
