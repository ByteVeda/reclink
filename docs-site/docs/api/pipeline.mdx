...
See the [Custom Plugins guide](/guides/custom-plugins) for details on registering blockers.
...
- [Evaluation](/api/evaluation) -- precision, recall, and F1 for measuring pipeline quality
- [Index Structures](/api/indexes) -- sub-linear search indexes used by LSH and q-gram blocking
- [String Metrics](/api/string-metrics) -- all available metric names for `compare_string`
- [Phonetic](/api/phonetic) -- phonetic algorithm details for `block_phonetic` and `compare_phonetic`
- [Custom Plugins](/guides/custom-plugins) -- registering custom blockers, comparators, and classifiers
