...
Available preprocessing operations include `fold_case`, `strip_punctuation`, `strip_diacritics`, `normalize_whitespace`, `normalize_unicode_nfkc`, `standardize_name`, and `remove_stop_words`. See the [Preprocessing API](/api/preprocessing) for the full list.
...
- **[Pipeline API](/api/pipeline)** -- Complete pipeline builder reference
- **[Evaluation API](/api/evaluation)** -- Precision, recall, F1, ROC curves
- **[Export API](/api/export)** -- CSV and JSON export functions
