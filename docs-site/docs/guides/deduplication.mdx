...
flowchart LR
    subgraph Without Blocking
        direction TB
        NB[N records] --> NB2["N × (N-1) / 2 pairs<br/>O(n²) comparisons"]
    end
    subgraph With Blocking
        direction TB
        B1[Block A] --> BA["pairs within A"]
        B2[Block B] --> BB["pairs within B"]
        B3[Block C] --> BC["pairs within C"]
    end
    NB2 -.-> |"100k records = 5 billion"| X1["Slow"]
    BA & BB & BC -.-> |"100k records ≈ 5 million"| X2["Fast"]
```
