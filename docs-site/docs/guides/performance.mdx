...
---
flowchart TD
    START[Need sub-linear search?] --> FIT{Dataset fits in RAM?}
    FIT -- No --> MMAP[MmapNgramIndex]
    FIT -- Yes --> EXACT{Need exact results?}
    EXACT -- No, approximate OK --> APPROX{Dataset size?}
    APPROX -- "< 100k" --> NGRAM[NgramIndex]
    APPROX -- "> 100k" --> MINHASH[MinHashIndex]
    EXACT -- Yes --> METRIC{Metric type?}
    METRIC -- "Edit distance" --> BK[BkTree]
    METRIC -- "Any metric" --> VP[VpTree]
```
